    ...
    <div className="min-h-[100dvh] bg-background pb-[calc(10rem+env(safe-area-inset-bottom))]">
      ...
      <div className="mx-auto max-w-lg space-y-4 px-4 pt-4">
        ...
        {booking.status === "balance_due" && (
          <div className="rounded-2xl border border-red-400/[0.30] bg-red-500/10 p-4 dark:bg-red-900/[0.25]">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="flex items-center">
                  <p className="text-sm font-bold text-red-700 dark:text-red-100">
                    Balance charge failed
                  </p>
                  <InfoTip
                    label="What happens next?"
                    triggerClassName="ml-1 inline-flex h-5 w-5 items-center justify-center rounded-full text-red-500/70 hover:text-red-600 dark:text-red-200/70 dark:hover:text-red-100 active:scale-90 transition-all"
                  >
          ...
          <section className="rounded-2xl border border-border bg-muted/30 p-4">
            ...
            {booking.completed_at && (
              <p className="text-sm text-foreground/80">
                <Clock size={14} className="inline -mt-0.5 mr-1.5 text-muted-foreground" />
                Marked complete{" "}
            ...
            {booking.cancellation_reason && (
              <p className="text-sm text-foreground/80">
                <AlertCircle size={14} className="inline -mt-0.5 mr-1.5 text-muted-foreground" />
        ...
        {recentMessages.length > 0 && booking.customer?.id ? (
          ...
          <Link
            ...
            className="flex items-center justify-center gap-2 rounded-2xl border border-border bg-muted/30 p-4 text-sm font-semibold text-foreground/80 transition-colors hover:bg-muted/60 active:scale-[0.99]"
          >
...
function Row({
  ...
  return (
    <li className="flex items-baseline justify-between gap-3">
      <span
        className={`text-sm ${bold ? "font-bold text-foreground" : accent ? "text-primary" : muted ? "text-muted-foreground" : "text-foreground/80"}`}
      >
        {label}
      </span>
      <span
        className={`shrink-0 text-sm tabular-nums ${bold ? "text-base font-bold text-foreground" : accent ? "font-bold text-primary" : muted ? "text-muted-foreground/60" : "text-foreground/90"}`}
      >
        {value}
      </span>
    </li>
