...
export function ShareSheet({
  ...
  const handleTap = useCallback(
    async (id: string) => {
      ...
      const emailSubject = encodeURIComponent("Order from me on KDER");
      ...
      switch (id) {
        ...
        case "x":
          window.open(
            `https://x.com/intent/tweet?text=${encodeURIComponent("Check out my plates on KDER!")}&url=${encodedUrl}`,
            "_blank"
          );
          break;
        case "facebook":
        ...
        case "email":
          window.open(
            `mailto:?subject=${emailSubject}&body=${encoded}`,
            "_self"
          );
          break;
        case "copy":
          ...
          if (navigator.share) {
            try {
              await navigator.share({
                title: "Order from me on KDER",
                text: "Check out my plates on KDER! Feed the city. Own your income.",
                url: fullUrl,
              });
            } catch {
              // User cancelled
      ...
      if (id !== "copy") onOpenChange(false);
    },
    [shareText, fullUrl, copyToClipboard, onOpenChange]
  );
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {/* App grid */}
        <div className="mt-5 overflow-x-auto pb-6 no-scrollbar">
          <div className="flex gap-4">
            {destinations.map((dest) => (
              <button
                key={dest.id}
                type="button"
                onClick={() => handleTap(dest.id)}
                className="flex flex-col items-center gap-2 active:scale-90 transition-transform"
                aria-label={`Share via ${dest.name}`}
              >
                <div
                  className={cn(
