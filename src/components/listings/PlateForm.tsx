...
export function PlateForm({ listing }: PlateFormProps) {
  ...
  const handleSave = async (status: ListingStatus) => {
    ...
    // Pre-order + delivery is now supported via self-delivery (migration
    // 025): the creator drives the order themselves on the pre-order date,
    // so the old Uber-courier "immediate dispatch" limitation no longer
    // applies. The prior pickup-only guard has been removed.
